...
{
	if (_x getVariable["blck_deleteAt",-1] < 0) then 
	{
	...
	//} else {
		//diag_log format["deleteAllDead: skipping unit %1 at time %2 with timestamp %3",_x,diag_tickTime,_x getvariable "blck_deleteAt"];
	};
}
forEach allDead;
