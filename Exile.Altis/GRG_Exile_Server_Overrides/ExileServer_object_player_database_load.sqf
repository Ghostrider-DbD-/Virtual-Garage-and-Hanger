...
private["_data","_oldPlayerObject","_playerUID","_sessionID","_position","_direction","_player","_clanID","_clanName","_clanData","_clanGroup",
"_devFriendlyMode","_devs","_requestingPlayer","_bambiPlayer","_headgear","_goggles","_binocular","_primaryWeapon","_handgunWeapon","_secondaryWeapon","_currentWeapon",
"_uniform","_vest","_backpack","_uniformContainer","_vestContainer","_backpackContainer","_assignedItems","_level7"];
...
// Initialize level7 trader elapsed time tracking.
// Test read of values from database
private _GRG_level7timeRemaining = format["loadPlayerLevel7:%1",_playerUID] call ExileServer_system_database_query_selectSingle;
if (typeName _GRG_level7timeRemaining isEqualTo "ARRAY") then {
	//diag_log format["converting array to scalar for _GRG_level7timeRemaining which now = %1",_GRG_level7timeRemaining];
	_GRG_level7timeRemaining = _GRG_level7timeRemaining select 0;
};
if (_GRG_level7timeRemaining < 0) then {_GRG_level7timeRemaining = 0};
_player setVariable["level7",_GRG_level7timeRemaining];
_player setVariable["level7LastChecked",diag_ticktime];
// Notify client of values
/*
	_msg = ["Duplicate",""];
	[_sessionID, "deployVehicleResponse", _msg] call ExileServer_system_network_send_to;
*/
[_sessionID,"updateLevel7TimeRemainingResponse", [_GRG_level7timeRemaining]] call ExileServer_system_network_send_to; 
//diag_log format["ExileServer_object_player_database_load: _level7 = %1 | typeName level7 = %3| lastChecked = %2",_GRG_level7timeRemaining,_player getVariable "level7LastChecked",typeName _GRG_level7timeRemaining];

//// Try to automatically delete duplicate records where one contains damage = 1.
format["deleteDeadPlayer:%1", _playerUID] call ExileServer_system_database_query_fireAndForget;
