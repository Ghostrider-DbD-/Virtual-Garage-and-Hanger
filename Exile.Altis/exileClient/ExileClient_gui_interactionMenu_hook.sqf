...
//diag_log format["ExileClient_gui_interactionMenu_hook: _object = 51 | time = %2 | _menus = %3",_object,time,_menus];
for "_i" from 0 to count(_menus) - 1 do 
{
	...
	if (_showMenu) then
	{
		...
		for "_n" from 0 to count(_menuItems) - 1 do 
		{
			...
			//diag_log format[" * ExileClient_gui_interactionMenu_hook: _menuItem = %1",_menuItem];
