		...
		_categoryItemClassNames = getArray(missionConfigFile >> "CfgTraderCategories" >> _categoryClass >> "items");
		{
			...
			if (_showItem) then
			{
				...
				switch (_quality) do
				{
					...
					case 7:		 { _qualityColor = [1,1,1,1]; }; // [0.11, 0.80, 0.60, 1]; };					
				};
				_playerMoney = player getVariable ["ExileMoney", 0];
				private _dlc = getText(configFile >> _configName >> _itemClassName >> "DLC");
				private _dlcIcon = modParams[_dlc,["logo"]];
				diag_log format["dlc = %3 | classname %1 | icon %2",_itemClassName,_dlcIcon,_dlc];
				if (_dlc isEqualTo "") then {_dlc = "A3"};
				//diag_log format["classname %1 | _salesPrice %2 | _requiredRespect %3 | _dlc %4",_itemClassName,_salesPrice,_requiredRespect,_dlc];
				if (_salesPrice <= _playerMoney && _requiredRespect <= ExileClientPlayerScore) then 
				{
					_storeListBox lbSetTooltip [_indexEntryIndex, format["DLC %1",_dlc]]; 
				};
		    	if (_salesPrice > _playerMoney) then
	    		{
	    			_popTabColor = [0.91, 0, 0, 0.6];
	    			_missingPopTabs = _salesPrice - _playerMoney;
	    			_storeListBox lbSetTooltip [_indexEntryIndex, format["DLC %1 | Missing %2 Pop Tabs",_dlc, _missingPopTabs call ExileClient_util_string_exponentToString]];
	    		};
	    		if (_requiredRespect > ExileClientPlayerScore) then
	    		{
	    			...
	    			_storeListBox lbSetTooltip [_indexEntryIndex, format["DLC %1 | Missing %2 Respect", _dlc, _missingRespect]];
	    		};
	    		if ((_salesPrice > _playerMoney) && (_requiredRespect > ExileClientPlayerScore)) then
	    		{
					_storeListBox lbSetTooltip [_indexEntryIndex, format["DLC %1 | Missing %2 Pop Tabs & %3 Respect",_dlc,_missingPopTabs call ExileClient_util_string_exponentToString, _missingRespect]];
		    	};
				...
				//_storeListBox lbSetPicture [_indexEntryIndex, getText(configFile >> "CfgMods" >> _itemclassName >> "logo")];
				...
				if !(_dlcIcon isEqualTo []) then 
				{
					...
					//_storeListBox lbSetPictureRight [_indexEntryIndex, _dlcIcon select 0];
		    		_storeListBox lbSetPictureRight [_indexEntryIndex, "exile_assets\texture\ui\poptab_trader_ca.paa"];					
				};
