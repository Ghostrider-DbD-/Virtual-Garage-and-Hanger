...
lbClear _itemListControl;
{
	...
	_categoryVehicleClassNames = getArray(missionConfigFile >> "CfgTraderCategories" >> _categoryClass >> "items");
	{
		...
		//diag_log format["className %1 _salesPrice %2 | _requiredRespect %3 | _dlc %4",_className,_salesPrice,_requiredRespect,_dlc];		
		switch (_quality) do
		{
			...
			case 7:		 { _qualityColor = [1,1,1,1]; }; //[0.11, 0.80, 0.60, 1]; };  //   #FF5733 11, 80%, 60%
		};
		if (_salesPrice <= _playerMoney && _requiredRespect <= ExileClientPlayerScore) then 
		{
			_itemListControl lbSetTooltip [_indexEntryIndex, format["DLC %1",_dlc]];
		};
		if (_salesPrice > _playerMoney) then
		{
			_popTabColor = [0.91, 0, 0, 0.6];
			_missingPopTabs = _salesPrice - _playerMoney;
			_itemListControl lbSetTooltip [_indexEntryIndex, format["DLC %1 | Missing %2 Pop Tabs",_dlc ,_missingPopTabs call ExileClient_util_string_exponentToString]];
		};
		if (_requiredRespect > ExileClientPlayerScore) then
		{
			...
			_itemListControl lbSetTooltip [_indexEntryIndex, format["DLC %1 | Missing %2 Respect", _dlc, _missingRespect]];
		};
		if ((_salesPrice > _playerMoney) && (_requiredRespect > ExileClientPlayerScore)) then
		{
			_itemListControl lbSetTooltip [_indexEntryIndex, format["DLC %3 | Missing %1 Pop Tabs & %2 Respect", _missingPopTabs call ExileClient_util_string_exponentToString, _missingRespect,_dlc]];
		};
