...
diag_log format["_VirtualBoatRack_network_VBR_GetStoredVehiclesResponse: _StoredVehicles = %1",_StoredVehicles];
StoredVehicles = [];
StoredVehiclesCount = count _StoredVehicles;
{
	...
	if (_className isKindOf 'Ship') then {StoredVehicles pushback _x};
